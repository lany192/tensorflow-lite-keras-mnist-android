 ...
 * 两条贯穿全文的约束：
 *
 * 1. **答案只能是非负整数且 ≤ [MAX_ANSWER]**。识别链路（FingerPaintView → InkSegmenter →
 *    MnistPreprocessor → KerasTFLite）只能读 0-9 十个数字，答案带小数点或分数线就无法判定；
 *    位数上限则来自"笔画宽/数字高"比例对识别精度的影响（见 MainActivity.MAX_DIGITS 的注释）。
 *    五六年级的题面因此允许出现小数、分数、百分数，但**答案一律构造为整数**。
 *
